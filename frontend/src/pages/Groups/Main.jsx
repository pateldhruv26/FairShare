...
export const Main = ({user,setUser,thememode,toggle,groupData,setgroupData}) => {
  console.log(groupData)
    ...
    const handleSendRequest = async()=>{
      try{
        const res= await axios.put(API_ENDPOINTS.SEND_REQUEST(user._id),{friendName})
    ...
    const handleSubmit =e=>{  
      ...
      const addgroup =async()=>{
          try{
            console.log(groupInput)
            const res = await axios.post(API_ENDPOINTS.CREATE_GROUP,{groupInput: { ...groupInput, groupCode: groupCode },})
    ...
    const handleJoin =e=>{  
      ...
      const addgroup =async()=>{
          try{
            console.log(JoingCode)
            const res = await axios.post(API_ENDPOINTS.JOIN_GROUP,{joincode})
            console.log(res.data.newgroup)
            setgroupflag((prev)=>!(prev))
            console.log(groupData)
          }catch(err){
    ...
    useEffect(()=>{
      ...
      const getGroups = async()=>{
        try{
          const res = await axios.get(API_ENDPOINTS.GET_GROUPS(user._id))//add user Id
