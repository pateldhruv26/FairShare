...
console.log(groupData)
    ...
    const handleSubmit = async()=>{
      try{
        const res= await axios.post(API_ENDPOINTS.SPLIT_BILL,{input})
  ...
  const handleApproved = async(memid)=>{
    try{
      const res=await axios.put(API_ENDPOINTS.MARK_APPROVED(groupData._id),{userId:memid})
   ...
   useEffect(()=>{
    const getMembers = async()=>{
      try{
        const res = await axios.get(API_ENDPOINTS.GET_MEMBERS(groupData._id))//add user Id
