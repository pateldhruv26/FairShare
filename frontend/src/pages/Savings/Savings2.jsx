  ...
  const handlesavedit = (e,obj) => {
    ...
    const editSavings = async () => {
      try {
        ...
        const res = await axios.put(API_ENDPOINTS.EDIT_SAVING(obj._id), {sav});
...
const currenciData = UCurrency(currenci);
  ...
  const handleAddSaving = async (e) => {
    try {
      ...
      const res = await axios.post(API_ENDPOINTS.ADD_SAVING, { saving });
...
// adding saving badge
  ...
  useEffect(()=>{
    ...
    const getSavings = async()=>{
      try{
        const res = await axios.get(API_ENDPOINTS.GET_SAVINGS(user._id))//add user Id
  ...
  const delsaving = async()=>{
    try{
        const res=await axios.delete(API_ENDPOINTS.DELETE_SAVING(selectedSavingId))
        console.log(res.data)
        setUpdateFlag((prev)=>!(prev))
