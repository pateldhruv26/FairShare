...
const Stocks = ({user,thememode,toggle}) => {
    ...
    const handleSubmit = async()=>{
        try{
          if(input!=""){
            const res=await axios.post(API_ENDPOINTS.ADD_STOCK(user._id),{input})
    ...
    const handledelete = async(stock)=>{
      try{
        const res = await axios.put(API_ENDPOINTS.DELETE_STOCK(user._id),stock)
    ...
    useEffect(()=>{
      const getStocks = async()=>{ try{ const res=await axios.get(API_ENDPOINTS.GET_STOCKS(user._id))
