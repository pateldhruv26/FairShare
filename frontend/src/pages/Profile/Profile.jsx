...
function Profile({ user, thememode, toggle,setUser}) {
  ...
  const handleUpload = () => {
    // Image upload functionality removed - Firebase storage not available
    alert("Image upload functionality is currently disabled. Please use a default profile picture.");
  };
  ...
  React.useEffect(() => {
    const getBadges = async () => {
      try {
        const response = await axios.get(API_ENDPOINTS.GET_BADGES(user._id));
