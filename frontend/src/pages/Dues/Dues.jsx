...
function Dues({ user, thememode, toggle,setUser }) {
  ...
  const mailsendstart = async () => {
    try {
      ...
      console.log(reqmail);
      const res = await axios.post(API_ENDPOINTS.SEND_START_MAIL, { reqmail });
  ...
  const mailsendrecurring = async (recurring) => {
    try {
      ...
      console.log(reqmail);
      const res = await axios.post(API_ENDPOINTS.SEND_MAIL_RECURRING, { reqmail, duedate, recurring });
      alert('Message Sent Successfully');
    } catch (err) {
...
// ----------------------- Submit ------------------- 
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(API_ENDPOINTS.ADD_BILL, { dueItem });
  ...
  useEffect(()=>{
    const getBills = async () => {
      try {
        const res = await axios.get(API_ENDPOINTS.GET_BILLS(user._id));
        console.log(res.data);
    ...
    try {
      ...
      if(Bill.amountedit===''||Bill.dueDateedit===''||Bill.titleedit===''||Bill.toWhomedit===''||Bill.userId===''){
        ...
        return;
              ...
              const res = await axios.put(API_ENDPOINTS.EDIT_BILL(obj._id), {Bill});
      console.log(res.data);
      setBill({
      userId: user._id,
      titleedit: '',
  ...
  const delBill = async () => {
    try {
      console.log(selecteddue);
              const res = await axios.delete(API_ENDPOINTS.DELETE_BILL(selecteddue));
      console.log(res.data);
      setbillflag((prev)=>!(prev))
